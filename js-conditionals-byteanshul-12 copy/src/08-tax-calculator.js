...
export function calculateTax(income) {
  ...
  if(income > 0 ){
    let taxedincome = Math.min(income , 10000)
    tax += taxedincome * 0 / 100
  }

  if ( income > 10000){
    let taxedincome = Math.min(income , 30000) - 10000
    tax += taxedincome * 10 / 100
  }

  if (income > 30000){
    let taxedincome = Math.min(income , 70000) - 30000 
    tax += taxedincome * 20 /100
  }

  if (income > 70000){
    let taxedincome = income - 70000
    tax += taxedincome * 30 / 100
  }
