...
export function canBorrowBook(memberAge, hasValidCard, overdueBooks) {
  ...
  let outputmessage = {
    allowed: hasValidCard,
    message: " "
  }


  if(memberAge >= 6 && hasValidCard == true && overdueBooks == 0){
    outputmessage.allowed = true,
    outputmessage.message = "You may borrow up to 3 books"
    }

    else if(memberAge < 6  && hasValidCard == true && overdueBooks == 0 ){
      outputmessage.allowed = false ;
      outputmessage.message = "Too young - must be at least 6 years old"
    }
      else if(memberAge >= 6 && hasValidCard == false && overdueBooks == 0 ){
        outputmessage.allowed =  false  
        outputmessage.message = "Invalid library card - please renew at the front desk"
        
      }

      else if (memberAge >= 6 && hasValidCard == true  && overdueBooks == `${overdueBooks}`){
        outputmessage.allowed = false
        outputmessage.message = `Please return your ${overdueBooks} overdue book(s) first`
        }


      else if(memberAge <= 5 && hasValidCard == false && overdueBooks != 0) {
        outputmessage.allowed = false
        outputmessage.message = "Too young - must be at least 6 years old"
        }
...
console.log(canBorrowBook(23,false ,1))